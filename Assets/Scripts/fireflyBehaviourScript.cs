...
public class fireflyBehaviourScript : MonoBehaviour
{
	...
	*/
    ...
    {
        p = new Process();
        target = GameObject.Find("Mother");
        if ("firefly1" == gameObject.tag)
        {
            GameObject player = GameObject.FindWithTag("FirstPlayer");
            FPSCam = player.transform.FindChild("MainCamera").gameObject;
        }
        else if ("firefly2" == gameObject.tag)
        {
            GameObject player = GameObject.FindWithTag("SecondPlayer");
            FPSCam = player.transform.FindChild("MainCamera").gameObject;
        }
    }
    ...
    void Update()
    {
        if (null == FPSCam)
        {
            print("NO REFERENCE TO THE PLAYER CAMERA !!!! Can't Work without it Sorry :(");
            return;
        }
